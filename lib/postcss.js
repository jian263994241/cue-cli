...
module.exports = function(content, file, conf) {
  ...
  if (file.isCssLike) {
    ...
    var cssprocess = postcss(postcssPlus).process(content, opts);

    content = cssprocess.css;

    cssprocess.root.walkAtRules('import', function(rule) {
      var subpath = rule.params.replace(/['"]/g, '');
      var extname = path.extname(subpath);
      if (extname == '') {
        subpath = subpath + '.less';
      };
      var depfile = path.join(file.dirname, subpath);
