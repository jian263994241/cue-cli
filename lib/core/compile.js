...
function postStandard(file,callback) {
  ...
  _.async.map(file.links,function(path,done){
    ...
    if(memoryCache[realpath]){
      linkFile = memoryCache[realpath];
      result.file = linkFile;
      result.ret = linkFile.getContentText();
      done(null,result);
    }else{
      linkFile = fis.file(realpath);
      exports(linkFile,function(file){
        if(file.links>0){
          exports(file,function(file){
            memoryCache[file.realpath] = file;
            result.file = file;
            result.ret = file.getContentText();
            done(null,result);
          })
        }else{
          memoryCache[file.realpath] = file;
