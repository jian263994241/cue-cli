 ...
 */
var exports = module.exports = function(file, callback) {
  if (!CACHE_DIR) {
...
function process(file, callback) {
  ...
  var standard = function(callback) {
    pipe(file, 'standard');
    postStandard(file, function(file){
      callback(null);
    });
  };
  var postprocessor = function(callback) {
    pipe(file, 'postprocessor');
    callback(null);
  };
  var optimizer = function(callback) {
    pipe(file, 'optimizer');
    callback(null);
  };
...
function browserifyPost(file, next) {
  if (file.browserify) {
    file.browserify.bundle(function(err, buf) {
      if (err) return console.log("\n"+err.stack);
      file.setContent(buf.toString());
      next(null);
    });
  } else {
    next(null);
 ...
 * @param  {file} file 文件对象
 */
function postStandard(file,callback) {
      ...
      try {
        switch (type) {
          ...
          case 'hash':
            ...
            //     exports(info.file, function(file) {
            //       unlock(info.file);
          ...
          case 'uri':
            ...
            if (info.file && info.file.isFile()) {
              ...
              file.addLink(info.file.realpath+"?uri");
              ret = "{uri}"+info.file.realpath+"{/uri}";
          ...
          case 'jsEmbed':
            ...
            if (f && f.isFile()) {
              file.addLink(f.realpath+"?embed");
              ret = "{embed}"+f.realpath+"{/embed}";
  ...
  // console.log(file.subpath,file.links);
  // console.log(file);

  //处理 link 文件
  _.async.map(file.links,function(path,done){
    var real = path.split('?');
    var realpath = real[0];
    var tag = real[1]
    var linkFile, result = {};
    ...
    result.id = realpath;
    result.tag = tag;
    linkFile = fis.file(realpath);

    file.cache.addDeps(linkFile.realpath);

    exports(linkFile,function(f){

      f.links.forEach(function(link){
        file.cache.addDeps(link.split('?')[0]);
      });
      exports(f,function(f){

        result.file = f;
        result.ret = f.getContentText();
    ...
    // file.cache.addDeps("b.inline.html");
    // file.cache.addDeps("c.inline.html");
    var url ;
    results.forEach(function(item){

      if(item.tag == "embed"){
        content = content.replace(new RegExp("{"+item.tag+"}"+ item.id +"{/"+item.tag+"}","g"),item.ret);
      }else{
        url = item.file.getUrl();
        content = content.replace(new RegExp("{"+item.tag+"}"+ item.id +"{/"+item.tag+"}","g"), url);
      }
