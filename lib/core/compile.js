...
/**
 * sourcemap - 根据file.sourcemaps生成sourcemap
 *
 * @param  {type} file fis.file文件对象
 */
function sourcemap(file){
  if(file.sourcemaps.length > 0){
    var sourcemap = '';
    _.each(file.sourcemaps, function(newMap){
      sourcemap = merge(sourcemap, newMap);
    });
    var sourcemapPath = file.getDeploy() + '.map';
    var content = file.getContent();
    _.write(sourcemapPath, sourcemap, 'utf8');
    if(content.match(/sourceMappingURL/)) return ;
    if(file.isJsLike){
      return file.setContent(content + '//@ sourceMappingURL=' + path.basename(sourcemapPath));
    }
    file.setContent(file.getContent() + '/*# sourceMappingURL=' + path.basename(sourcemapPath) + '*/');
  }
}
