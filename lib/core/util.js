...
_.serverInfo = function () {
  try {
    var serverInfo = require(fis.project.getTempPath('server/conf.json'));
  } catch (e) {
    console.log('无法获取 server info');
  }
  return serverInfo;
}

// 异步程序hack, srcFix需要修复连接的文件
_.asyncFix = function (file) {
  var rFile = file.getDeploy();
  ...
  return function (content, srcFix) {

    var file = fis.file.wrap(origin);
    _.del(rFile);
    file.setContent(content);
    file.save();
    var nUrl = file.getUrl();
    var _file, info;
    for (var key in srcFix) {
      info = pth.parse(key);
      _file = srcFix[key];
      if (info.ext != '.html') continue;
      _file.links.forEach(function (item) {
        if (item == file.subpath) {
          _file.replace(rUrl, nUrl);
          _file.save();
        };
      })
    };
    if (fis._live) {
      var serverInfo = _.serverInfo();
      var socket = io('http://127.0.0.1:' + serverInfo.port + '/browser-sync-cp', {
