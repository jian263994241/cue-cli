 ...
 * @property {Array} sourcemaps 存放sourcemap
 *
 *
 * @class
 * @inner
...
var File = Object.derive(function() {
  ...
  if (subpath) {
    if (this.release === false && !this.url) {
      ...
      //release & url
      var reg = new RegExp('(' + _.escapeReg(rExt) + '|.entry){0,1}' + _.escapeReg(ext) + '$|' + _.escapeReg(rExt) + '$', 'i');
      var release;
      if (this.release) {
   ...
   ** _.write 的快捷方式
   */
  save: function(realpath) {
    realpath = realpath || this.getDeploy();
    _.write(realpath, this._content);
    return this;
  },
  /*
   ** 返回获取编译目录文件
   ** rpath:true 返回获取编译目录路径
   */
  getDeploy: function(rpath) {
    var argv = fis.argv;
    var www;
    if (argv.d) {
      www = path.join(process.cwd(), argv.d)
    } else if (this.deploy) {
      if(_.isArray(this.deploy)){
        var deploy = _.findLast(this.deploy, function(n){
          return n.to;
        })
        if(deploy){
          www = path.resolve(process.cwd(), deploy.to);
        }else{
          www = fis.project.getTempPath('www');
        }
      }else{
        www = path.resolve(process.cwd(), this.deploy.to);
      }
    ...
    if (rpath) {
      ...
      return path.join(www, this.getHashRelease());
    };
  },
  /*
   ** 替换file content 内容
   */
  replace: function(_old, _new) {
    var savePath = this.getDeploy();
    var content = this._content;

    _old = path.basename(_old);
    _new = path.basename(_new);

    this._content = content.replace(new RegExp(_old, 'g'), _new);
  },
  /**
